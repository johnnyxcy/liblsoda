#ifndef lsoda_internal_h
#define lsoda_internal_h

int stoda(struct lsoda_context_t *ctx, double *y, int jstart);
int correction(struct lsoda_context_t *ctx, double *y, double pnorm, double *del,
    double *delp, double told, int *m);
int prja(struct lsoda_context_t *ctx, double *y);

int solsy(struct lsoda_context_t *ctx, double *y);
int orderswitch(struct lsoda_context_t *ctx, double rhup, double dsm, double *rh,
    int kflag, int maxord);
...
#define ERROR(fmt, ...)                       \
    (ctx->error ? free(ctx->error) : (void)1, \
        ctx->error =                          \
            _strdup_printf("EE:" fmt " @(%s:%d)", ##__VA_ARGS__, __FILE__, __LINE__))

#endif /* lsoda_internal_h */