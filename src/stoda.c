...
int stoda(struct lsoda_context_t *ctx, double *y, int jstart)
{
    ...
    */
    ...
    /*
       ...
       If _C(h) or _C(meth) is being changed, _C(ialth) is reset to (_C(nq) + 1) = _C(nq)
       + 1 to prevent further changes in _C(h) for that many steps.
    */
    if(jstart == -1) {
        _C(ipup) = _C(miter);
        if(_C(ialth) == 1)
    ...
    */
    ...
    while(1) {
        ...
        */
        _C(jcur) = 0;
        while(1) {
            if(fabs(_C(rc) - 1.) > __lsoda_common_CCMAX)
                _C(ipup) = _C(miter);
            if(_C(nst) >= _C(nslp) + __lsoda_common_MSBP)
                _C(ipup) = _C(miter);
            _C(tn) += _C(h);
            for(j = _C(nq); j >= 1; j--)
        ...
        if(dsm <= 1.) {
            /*
               ...
               by 1. If _C(ialth) is then 1 and _C(nq) < maxord, then _C(acor) is saved
               for use in a possible order increase on the next step. If a change in _C(h)
               is considered, an increase or decrease in order by one is considered also.
               A change in _C(h) is made only if it is by a factor of at least 1.1.  If
               not, _C(ialth) is set to 3 to prevent testing for that many steps.
            */
            kflag = 0;
            _C(nst)++;
            _C(hu)    = _C(h);
