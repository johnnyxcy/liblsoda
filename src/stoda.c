  ...
  */
  ...
  while (1) {
    ...
    */
    _C(jcur) = 0;
    while (1) {
      if (fabs(_C(rc) - 1.) > __lsoda_common_CCMAX) _C(ipup) = _C(miter);
      if (_C(nst) >= _C(nslp) + __lsoda_common_MSBP) _C(ipup) = _C(miter);
      _C(tn) += _C(h);
      for (j = _C(nq); j >= 1; j--)
    ...
    */
    ...
    if (dsm <= 1.) {
      /*
         ...
         is considered also. A change in _C(h) is made only if it is by a factor
         of at least 1.1.  If not, _C(ialth) is set to 3 to prevent testing for
         that many steps.
      */
      kflag = 0;
      _C(nst)++;
