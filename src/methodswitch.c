...
void methodswitch(struct lsoda_context_t *ctx, double dsm, double pnorm,
  ...
  */
  if (_C(meth) == 1) {
    if (_C(nq) > 5) return;
    if (dsm <= (100. * pnorm * __lsoda_common_ETA) || _C(pdest) == 0.) {
      if (_C(irflag) == 0) return;
      rh2 = 2.;
      nqm2 = fmin(_C(nq), mxords);
    } else {
      ...
      if (rh2 < __lsoda_common_RATIO * rh1) return;
    }
    /*
  ...
  */
  ...
  if ((rh1 * __lsoda_common_RATIO) < (5. * rh2)) return;
  alpha = fmax(0.001, rh1);
  dm1 *= pow(alpha, exm1);
  if (dm1 <= 1000. * __lsoda_common_ETA * pnorm) return;
  /*
